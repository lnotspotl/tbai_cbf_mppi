...
class VanillaSafetyFilter:
  def __init__(self, system: SimpleSingleIntegrator2D, cbf: tbai_cbf_mppi.cbf.ControlBarrierFunctionNew, alpha: float):
    self.system = system
    self.cbf = cbf
    self.alpha = alpha
...
class VanillaSafetyFilterNew:
  def __init__(self, A, B, cbf, alpha):
    assert isinstance(cbf, tbai_cbf_mppi.cbf.ControlBarrierFunctionNew), "cbf must be a ControlBarrierFunctionNew"
    assert A.shape == (2, 2)
    assert B.shape == (2, 2)
    self.A = A
    self.B = B
    self.cbf = cbf
    self.cbf_casadi = tbai_cbf_mppi.cbf.CasadiControlBarrierFunction(self.cbf)
    self.alpha = alpha
...
if __name__ == "__main__":
  factory = tbai_cbf_mppi.cbf.ControlBarrierFunctionFactory()
