...
from tbai_cbf_mppi.utils import remove_kwargs
import tbai_cbf_mppi.ulogging
...
logger = tbai_cbf_mppi.ulogging.get_logger(__name__)
...
def cost_fn(fn: Callable, backend: str = None, dtype: str = None, locals: dict = None, globals: dict = None):
  if hasattr(fn, "_tbai_cbf_mppi_backend"):
    return fn
  ...
  if backend == "numpy":
    ...
    jitted_fn._tbai_cbf_mppi_backend = backend
  elif backend == "cupy":
  ...
  jitted_fn._tbai_cbf_mppi_source = source
  jitted_fn._tbai_cbf_mppi_new_source = new_source
...
def get_cost_function_parameterized(
  ...
  assert hasattr(stage_cost, "_tbai_cbf_mppi_backend"), "Stage cost must be jitted with cost_fn"
  assert hasattr(terminal_cost, "_tbai_cbf_mppi_backend"), "Terminal cost must be jitted with cost_fn"
  assert hasattr(stage_cost, "_tbai_cbf_mppi_source"), "Stage cost must be jitted with cost_fn"
  assert hasattr(terminal_cost, "_tbai_cbf_mppi_source"), "Terminal cost must be jitted with cost_fn"
  assert hasattr(stage_cost, "_tbai_cbf_mppi_new_source"), "Stage cost must be jitted with cost_fn"
  assert hasattr(terminal_cost, "_tbai_cbf_mppi_new_source"), "Terminal cost must be jitted with cost_fn"
